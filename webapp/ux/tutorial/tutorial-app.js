(() => {
  ...
  function renderExploreBridge(slide) {
    const bridges = {
      2: ['Voir des cas d’agents', '../../explore/?solution_type=agent'],
      6: ['Comparer les cas de chatbot et d’agent', '../../explore/?solution_type=agent'],
      7: ['Explorer les cas documentés', '../../explore/?evidence_level=documented'],
      8: ['Voir les agents avec un cadre de contrôle', '../../explore/?solution_type=agent'],
      9: ['Voir les cas de connaissance et support', '../../explore/?business_function=customer-service']
    };
    const bridge = bridges[slide.id];
    if (!bridge) return '';
    return `<a class="tuto-explore-bridge" href="${bridge[1]}"><span>EXPLORE · CATALOGUE</span><strong>${escapeHtml(bridge[0])} <b aria-hidden="true">↗</b></strong></a>`;
  }
